...
class _DashboardTabState extends State<DashboardTab>
  ...
  _getData() async {
    if (widget.type == "day") _dashboardBloc.add(GetStatisticDay());
    if (widget.type == "month") _dashboardBloc.add(GetStatisticMonth());
    if (widget.type == "year") _dashboardBloc.add(GetStatisticYear());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<DashboardBloc, DashboardState>(
      ...
      if (widget.type == "day") {
        _static = state.statisticDay ?? const StatisticEntity();
        _errorMessage = state.errorMessageDay;
      }
      if (widget.type == "month") {
        _static = state.statisticMonth ?? const StatisticEntity();
        _errorMessage = state.errorMessageMonth;
      }
      if (widget.type == "year") {
        _static = state.statisticYear ?? const StatisticEntity();
                  ...
                  children: [
                    ...
                    Expanded(
                      ...
                      child: _boxWidget(Column(
                        children: [
                          Text(
                            "key_total_revenue".tr(),
                            style: AppTextStyles.textSize10(
                                color: AppColor.greyColor),
                          ),
                          5.w,
                          Text(
                            Helper.formatCurrencyVND(_static.totalRevenue ?? 0),
                            style: AppTextStyles.textSize14(),
                          ),
                        ],
                      )),
